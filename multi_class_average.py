...
def perceptron_learn(weight_vector, x_t, y_t):
    ...
    global learning_mistakes, learning_successes, count, cached_weight_vector
    y_hat = arg_max(x_t, weight_vector)
    if y_t != y_hat:
        #mistake, update weight vector
        learning_mistakes += 1
        cached_weight_vector = np.add(cached_weight_vector, np.subtract(f_x_y(x_t, y_t), f_x_y(x_t, y_hat)))
        weight_vector = np.add(weight_vector, [count * x for x in np.subtract(f_x_y(x_t, y_t), f_x_y(x_t, y_hat))])
        count = 1
    else:
        count += 1
...
for fold in range(1):
    ...
    weight_vector_list = []
    cached_weight_vector = np.asarray([0] * VECTOR_SIZE * NUM_CLASSES)
    cached_weight_vector_list = []
    ...
    for iteration in range(TRAINING_ITERATIONS):
        ...
        weight_vector = [float(x/total_count) for x in weight_vector]
        weight_vector_list.append(weight_vector[:])
        cached_weight_vector_list.append(cached_weight_vector[:])
